...
function inCheck(f){
	...
	if(f.chkpasswd.value==""){
		alert("비밀번호 확인을 입력해 주세요");
		f.repasswd.focus();
		return false;
	}
	if(f.chkpasswd.value!=f.passwd.value){
...
<FORM 
	name='frm' method='POST' action='./prcreateProc.jsp' enctype="multipart/form-data" onsubmit="return inCheck(this)">
  <TABLE>
    <TR>
      <TH>아이디</TH>
      <TD><input type="text" name="id" size="15" placeholder="아이디">
      <button type="button" onclick="idCheck(document.frm.id.value)">중복 확인</button></TD>
    </TR>
    <TR>
      <TH>비밀번호</TH>
    ...
    <TR>
      <TH>비밀번호 확인</TH>
      <TD><input type="password" name="chkpasswd" size="15" placeholder="비밀번호 확인"></TD>
    </TR>
    <TR>
      <TH>이름</TH>
    ...
    <TR>
      <TH>전화번호</TH>
      <TD><input type="text" name="tel" size="15" placeholder="전화번호(선택)"></TD>
    </TR>
    <TR>
      <TH>이메일</TH>
      <TD>
      <input type="email" name="email" size="15" placeholder="이메일">
	  <button type="button" onclick="emailCheck(document.frm.email.value)">중복 확인</button></TD>
    </TR>
    <TR>
      <TH>우편번호</TH>
      <TD>
      <input type="text" name="zipcode" id="sample6_postcode" placeholder="우편번호(선택)" size="15">
	  <button type="button" onclick="sample6_execDaumPostcode()">주소 검색</button></TD>
    </TR>
    <TR>
      <TH>주소</TH>
      <TD>
      <input type="text" name="address1" size="35" id="sample6_address"  placeholder="주소(선택)"><br>
	  <input type="text" name="address2" size="35" id="sample6_address2" placeholder="상세주소(선택)">
	  </TD>
    </TR>
    <TR>
      <TH>직업</TH>
	...
	<div class="fileBox">
		...
		<input type="file" name="uploadBtn" id="uploadBtn" class="uploadBtn" accept=".jpg,.png,.gif">
	</div>
	* jpg, png, gif만 가능
